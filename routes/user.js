 ...
 * @LastEditTime: 2023-09-21 11:02:17
 */
...
router.get("/register2", function (req, res, next) {
    new UserModel({ isAdmin: false, username: "test", password: "5555", age: 15, nickname: "155", emai: "233", adress: "888", school: "000", schedule: "555" }).save((err, newUser) => {
...
router.post("/register", function (req, res, next) {
    ...
    if (verify) {
        const {
            params: { username, password, age, nickname, emai, adress, school, schedule },
        } = verify;
        UserModel.findOne({ username }).then((user) => {
            if (!user) {
                new UserModel({ isAdmin: username == "niumengfei" ? true : false, username, password, age, nickname, emai, adress, school, schedule }).save((err, newUser) => {
                    if (err) return console.error(err);
                    res.send({
                        code: "1",
                        data: {},
    ...
    if (verify) {
        ...
        UserModel.findOne({ username, password }).then((user) => {
            if (user) {
                ...
                let encrypt = Utils.encrypt.DynamicDES(
                    JSON.stringify({
                        ...user._doc,
